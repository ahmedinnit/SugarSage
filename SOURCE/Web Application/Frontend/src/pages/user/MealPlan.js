...
export default function MealPlan() {
  ...
  const handleSubmit = () => {
    ...
    // First check if the user can generate a meal plan
    axios
      .get(`http://localhost:3001/api/user/mealplans/check/${user_id}`, {
        headers: {
          Authorization: `${token}`
        }
      })
      .then((checkResponse) => {
        if (checkResponse.data.success) {
          // If user can generate a meal plan, proceed with generating it
          axios
            .post(
            ...
            .then((generateResponse) => {
              console.log('Meal plan generated:', generateResponse.data);
              setOpen(false);
              setLoading(false); // Stop loading
              navigate('/user/mealplan/choose', { state: { data: generateResponse.data } }); // Replace with your target route
            })
            .catch((error) => {
          ...
          // If user cannot generate a meal plan, show error message
          setError(checkResponse.data.message || 'Meal plan for today already exists.');
          setLoading(false); // Stop loading
        }
      })
      .catch((error) => {
        ...
        setLoading(false); // Stop loading in case of error
        setError('Meal Plan already exists.');
      });
  };

  const handleViewMealPlan = () => {
    navigate('/user/mealplan/view');
  };
    ...
    <Box sx={{ flexGrow: 1, p: { xs: 2, sm: 4 } }}>
      ...
      <MainCard elevation={3} sx={{ p: { xs: 2, sm: 10 } }}>
        <Grid container direction="column" alignItems="center" spacing={2}>
          ...
          <Grid item xs={12}>
            <MealPlanButton title="View Meal Plan" onClick={handleViewMealPlan} />
          </Grid>
        </Grid>
      </MainCard>
